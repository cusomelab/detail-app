  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.message && error.message.includes("Requested entity was not found")) {
        alert("API Key가 유효하지 않습니다. 다시 확인해주세요.");
        setHasApiKey(false);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center border border-gray-100">
           ...
           <p className="text-gray-500 mb-6">Google AI Studio에서 발급받은 API 키를 입력하세요.<br/><span className="text-xs text-gray-400">입력한 키는 브라우저에만 저장됩니다.</span></p>
           <input
             type="password"
             value={apiKeyInput}
           ...
           />
           <button onClick={handleApiKeySubmit} className="w-full py-3 px-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold transition-colors shadow-lg flex items-center justify-center gap-2">
